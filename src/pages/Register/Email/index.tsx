...
const Email: React.FC<StepProps> = ({ setAlertMsg, setOpenSnackbar, setAlertSeverity }) => {
  const [email, setEmail] = useState('')
  const [loading, setLoading] = useState(false)
  const [errorType, setErrorType] = useState('')
  const { temporaryUser: temporaryUserData, setTemporaryUser } = useTemporaryUser()
  ...
  const handleFormSubmit: FormEventHandler = async (e) => {
    ...
    if (!temporaryUserData.role) {
      setAlertMsg('Reinicie o registro.')
      setAlertSeverity('error')
      setOpenSnackbar(true)
      return
    }
    const { temporaryUser, error } = await createTemporaryUser({ email, role: temporaryUserData.role })
    if (error) {
      switch (error) {
